      ...
      <div className="card all-em-card">
        <div className="all-e-img">
          <NavLink to={`/employee/details/${props.details.employeeID}`}>
            <img
              src={(props.details.profileImage != "") ? (process.env.REACT_APP_PUBLIC_PATH + props.details.profileImage) : (ProfileImage)}
              className="img-fluid"
              alt="Profile"
            />
          </NavLink>
          <h5 className="card-title">
            <NavLink to={`/employee/details/${props.details.employeeID}`}>{props.details.name}</NavLink>
          </h5>
          <p className="mb-0">{props.details.designation?.name}</p>
        </div>
